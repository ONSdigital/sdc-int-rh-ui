...
class TestServiceCalls(RHTestCase):
    ...
    def test_client_ip_valid(self):
        ...
        self.assertEqual(ServiceCalls.single_client_ip(valid_request), expected_valid)
        self.assertEqual(ServiceCalls.single_client_ip(spoofed_request), expected_valid)
        self.assertEqual(ServiceCalls.single_client_ip(spoofed_request_ipv6), expected_valid)
        self.assertEqual(ServiceCalls.single_client_ip(unknown_request), expected_valid)
    ...
    def test_client_ip_invalid_single(self):
        ...
        with self.assertLogs('respondent-home', 'WARN') as cm:
            self.assertEqual(ServiceCalls.single_client_ip(single_ip_request), expected_empty)
    ...
    def test_client_ip_invalid_ipv6(self):
        ...
        expected_empty = ''
        with self.assertLogs('respondent-home', 'WARN') as cm:
            self.assertEqual(ServiceCalls.single_client_ip(invalid_request_ipv6), expected_empty)
