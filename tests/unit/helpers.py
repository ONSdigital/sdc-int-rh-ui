...
class TestHelpers(RHTestCase):
    ...
    async def check_post_enter_address(self, display_region):
        with self.assertLogs('respondent-home', 'INFO') as cm, \
                mock.patch('app.service_calls.aims.Postcode.get_ai_postcode') as mocked_get_ai_postcode:
    ...
    async def check_post_enter_address_finder(self, display_region, region):
        with self.assertLogs('respondent-home', 'INFO') as cm, mock.patch(
                'app.service_calls.rhsvc.Cases.get_cases_by_uprn') as mocked_get_case_by_uprn:
            if region == 'W':
    ...
    async def check_post_select_address(self, display_region, region):
        with self.assertLogs('respondent-home', 'INFO') as cm, \
                mock.patch('app.service_calls.aims.Postcode.get_ai_postcode') as mocked_get_ai_postcode, mock.patch(
                'app.service_calls.rhsvc.Cases.get_cases_by_uprn') as mocked_get_case_by_uprn:
            mocked_get_ai_postcode.return_value = self.ai_postcode_results
            if region == 'W':
                mocked_get_case_by_uprn.return_value = self.rhsvc_case_by_uprn_hh_w
    ...
    async def check_post_confirm_send_by_text_error_from_request_fulfilment(self, display_region):
        with self.assertLogs('respondent-home', 'INFO') as cm, \
                mock.patch('app.service_calls.rhsvc.Fulfilments.get_fulfilment') as mocked_get_fulfilment, \
    ...
    async def check_post_confirm_send_by_email(self, display_region, region):
        with self.assertLogs('respondent-home', 'INFO') as cm, mock.patch(
            ...
            'app.service_calls.rhsvc.Fulfilments.request_fulfilment_email'
    ...
    async def check_post_confirm_send_by_post_input_yes(self, display_region, region):
        with self.assertLogs('respondent-home', 'INFO') as cm, \
                mock.patch('app.service_calls.rhsvc.Fulfilments.get_fulfilment') as mocked_get_fulfilment, \
                mock.patch('app.service_calls.rhsvc.Fulfilments.request_fulfilment_post') as mocked_request_fulfilment_post:
